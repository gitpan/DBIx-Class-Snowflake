...
Base name of attribute on name of column and name of table 'table.column'
=head1 TODO
...
There is one known limitation.  The module understands an attribute as the
name of the relationship that got it there and the name of the column.  So,
for example, if you had two classes, DimDate and FactSale, and FactSale had
a relationship with DimDate called date_id and DimDate had an attribute called
day_of_week this module would assume that date_id.day_of_week was unique.  If
there is some other dimension that would translate to date_id.day_of_week
there is no guarantee as to which one would be picked.
